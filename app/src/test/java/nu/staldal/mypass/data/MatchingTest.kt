...
/**
 * The desktop `MyPass`'s browser-integration matching tests, ported one for one.
...
class MatchingTest {
     ...
     * version desktop MyPass depends on, so this test is what keeps the two sides
    ...
    fun entryWithoutUrlNeverMatches() {
        ...
        val entries = listOf(
            PasswordEntry(name = "github.com", username = "user", password = Secret("MyPass"))
        )
    ...
    fun unknownTopLevelDomainsStillAllowParentMatching() {
        ...
        // its own subdomains. Guava stops at the list, so MyPass fills this in;
        // without it `.example`, `.internal`, `.home` and every other unlisted
        // TLD would behave differently here than in desktop MyPass.
        assertEquals(
            listOf("evil.example"),
            matches("github.com.evil.example", "evil.example"),
        )
