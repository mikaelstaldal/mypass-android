...
class ScryptFormatTest {
    ...
    fun deviceDefaultsAndEveryOfferedCostPassValidationWithoutKdf() {
        for ((heapMiB, expectedMax) in listOf(128 to 15, 192 to 16, 256 to 16, 512 to 17)) {
            ...
            for (logN in nu.staldal.mypass.data.ScryptDefaults.MIN_LOG_N..max) {
        ...
        assertEquals(ScryptFormat.maxSupportedLogN(), nu.staldal.mypass.data.ScryptDefaults.LOG_N)
        assertEquals(ScryptFormat.maxSupportedLogN(), nu.staldal.mypass.data.ScryptDefaults.MAX_LOG_N)
    }
