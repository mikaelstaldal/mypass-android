...
/**
 * Input validation, the same rules as the desktop `MyPass` so a vault written by
...
object Validation {
    ...
    fun validateName(name: String) {
        if (name.isEmpty()) {
            throw MyPassException.InvalidInput("entry name", "must not be empty")
        }
    ...
    fun validateUrl(url: String) {
        if (url.isEmpty()) throw MyPassException.InvalidInput("url", "must not be empty")
        validateText("url", url)
    }

    /** The optional `realm` matching hint, under the same rules as `url`. */
    fun validateRealm(realm: String) {
        if (realm.isEmpty()) throw MyPassException.InvalidInput("realm", "must not be empty")
    ...
    fun validateEntries(entries: List<PasswordEntry>) {
        ...
        for ((index, entry) in entries.withIndex()) {
            try {
                validateEntry(entry)
            } catch (e: MyPassException.InvalidInput) {
                throw MyPassException.InvalidInput("vault entry ${index + 1} of ${entries.size} (${e.what})",
                    "${e.reason}; export the encrypted vault and correct its metadata in desktop MyPass before importing")
            }
            if (!names.add(entry.name)) {
                throw MyPassException.InvalidInput("vault", "duplicate entry name at entry ${index + 1} of ${entries.size}; export the encrypted vault and rename entries in desktop MyPass before importing")
            }
        }
    }
    ...
    private fun validateText(what: String, value: String) {
        if (value.codePointCount(0, value.length) > MAX_NAME_LEN) {
            throw MyPassException.InvalidInput(what, "longer than $MAX_NAME_LEN characters")
        }
        ...
        while (i < value.length) {
            ...
            if (Character.isISOControl(codePoint)) {
                throw MyPassException.InvalidInput(what, "contains control characters")
            }
