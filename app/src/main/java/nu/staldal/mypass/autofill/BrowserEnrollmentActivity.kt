...
class BrowserEnrollmentActivity : FragmentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val candidateToken = intent.data
            ?.takeIf { it.scheme == "mypass-browser-enrollment" && it.authority == "request" }
                    ...
                    text = {
                        Text(buildString {
                            ...
                            if (destination.packageName in Browsers.KNOWN) {
                                ...
                                append("but its publisher certificate does not match MyPass's pin.")
                            }
                        })
                    },
    ...
    private fun decide(trust: Boolean) {
        ...
        val app = application as MyPassApplication
    ...
    private fun respond(app: MyPassApplication, request: EnrollmentDestination) {
...
private object MatchingHost {
    fun accepts(destination: EnrollmentDestination, form: ParsedForm): Boolean =
        nu.staldal.mypass.data.Matching.eligibleWebHost(form.webScheme, form.webDomain) == destination.host &&
