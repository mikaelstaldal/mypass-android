...
import nu.staldal.mypass.MainActivity
 ...
 *
 * Shared by [MyPassAutofillService] and [AutofillAuthActivity], which build the
...
object FillResponses {
    ...
    fun browserEnrollment(
        ...
        val intent = Intent(context, BrowserEnrollmentActivity::class.java)
            .setData(Uri.Builder().scheme("mypass-browser-enrollment").authority("request")
     ...
     *
     * Nothing is released here, and nothing about the vault is revealed — MyPass
    ...
    fun locked(
        ...
        val intent = Intent(context, AutofillAuthActivity::class.java)
            .setData(Uri.Builder().scheme("mypass-autofill-auth").authority("request").appendPath(token).build())
        ...
        val inlinePresentation =
            if (inline != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                inlinePresentation(
                    ...
                    // inline suggestion. Opening MyPass itself is the honest
     ...
     * desktop MyPass shares, and this is the only place to refuse: a `SaveRequest`
