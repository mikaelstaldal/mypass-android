...
import nu.staldal.mypass.autofill.BrowserCertificates
import nu.staldal.mypass.autofill.FillDiagnostics
import nu.staldal.mypass.autofill.Browsers
import nu.staldal.mypass.data.ScryptDefaults
...
import nu.staldal.mypass.util.Biometrics
...
fun SettingsScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            SwitchSetting(
                label = "Lock when MyPass leaves the screen",
            ...
            NumberSetting(
                ...
                help = "0 leaves the clipboard untouched. A clipboard history " +
                    "manager may keep a copy MyPass cannot reach.",
            ...
            Text(
                if (autofillEnabled) {
                    "MyPass is the autofill service. It fills login forms in a browser " +
                        "only, on https: sites, and only from entries that have a url."
                } else {
                    "MyPass is not the autofill service. Turn it on to fill login forms " +
                        "in your browser."
                },
            ...
            OutlinedButton(
                ...
                Text(if (autofillEnabled) "Autofill settings" else "Use MyPass for autofill")
            }
            ...
            Spacer(Modifier.height(8.dp))
            OutlinedButton(
                onClick = { exportVaultLauncher.launch("mypass.scrypt") },
                Modifier.fillMaxWidth(),
            ) {
                Text("Export encrypted vault (mypass.scrypt)")
            }
            Text(
                "The file as it is on disk. `scrypt dec mypass.scrypt` on a desktop " +
                    "reads it, and so does desktop MyPass.",
                style = MaterialTheme.typography.bodySmall,
            )
            Spacer(Modifier.height(8.dp))
            OutlinedButton(
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        ...
                        exportJsonLauncher.launch("mypass-export.json")
                    }
                ) { Text("Export") }
            },
            dismissButton = {
...
private fun FillDiagnosticsSetting(enabled: Boolean, onEnabled: (Boolean) -> Unit) {
    SwitchSetting(
        ...
        help = "Keeps the last ${FillDiagnostics.CAPACITY} autofill requests in memory: the " +
            ...
            "names. Also writes the same metadata to Logcat with tag mypass-autofill while enabled; " +
            "forgotten in MyPass when switched off or when MyPass stops.",
        checked = enabled,
        onChecked = onEnabled,
    )
    if (!enabled) return
    ...
    pending?.let { (pkg, identity) ->
        AlertDialog(
            ...
            text = { Column {
                ...
                if (pkg in Browsers.KNOWN) Text("This adds an acceptable signer alongside MyPass's built-in publisher pins.")
