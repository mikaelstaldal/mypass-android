 ...
 * the vault. A write happens here instead, in pw's own UI, with the vault open
...
class AutofillSaveActivity : FragmentActivity() {
    ...
    companion object {
        ...
        const val EXTRA_USERNAME = "nu.staldal.pw.autofill.USERNAME"
        const val EXTRA_PASSWORD = "nu.staldal.pw.autofill.PASSWORD"
    }
}
...
private fun SaveDialog(
    ...
    AlertDialog(
        ...
        title = { Text(if (match == null) "Save login in pw?" else "Update login in pw?") },
        text = {
            Column {
                Text(
                    if (match == null) {
