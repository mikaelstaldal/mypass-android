...
/** Read-only conversion from a KeePass KDBX database to pw entries. */
object KeePassImport {
    ...
    fun decode(input: InputStream, passphrase: String): List<PasswordEntry> {
        ...
        val database = try {
            ...
            throw PwException.InvalidInput("KeePass file", "needs too much memory to import")
        } catch (_: StackOverflowError) {
            throw PwException.InvalidInput("KeePass file", "is nested too deeply")
        } catch (e: Exception) {
    ...
    private object BoundedKdf : KdfProvider {
        ...
        private fun argon2(parameters: KdfParameters.Argon2, compositeKey: ByteArray): ByteArray {
            ...
            ) throw PwException.InvalidInput("KeePass file", "uses an unsupported Argon2 version")
        ...
        private fun resourceLimit(): Nothing =
            throw PwException.InvalidInput("KeePass file", "requests excessive key-derivation resources")
    }
    ...
    internal fun entries(database: KeePassDatabase): List<PasswordEntry> {
        ...
        while (pending.isNotEmpty()) {
            ...
            if (depth > MAX_GROUP_DEPTH) {
                throw PwException.InvalidInput("KeePass file", "contains groups nested more than $MAX_GROUP_DEPTH levels")
            }
        ...
        imported.forEach { entry ->
            ...
            if (!names.add(entry.name)) throw PwException.AlreadyExists(entry.name)
        }
        return imported
    }
        ...
        try {
            while (true) {
                ...
                if (out.size() + count > MAX_FILE_BYTES) {
                    throw PwException.InvalidInput("KeePass file", "is larger than $MAX_FILE_BYTES bytes")
                }
                out.write(buffer, 0, count)
            }
            return out.toByteArray()
        } catch (e: IOException) {
            throw PwException.InvalidInput("KeePass file", "could not read it: ${e.message ?: "I/O error"}")
        } finally {
            buffer.fill(0)
        }
    }
